...
export function evaluateClaimValidity(message: string, score: ScbkrScore, f: ExtractedFeatures): ClaimValidityResult {
  ...
  if (!f.hasSubjectIdentity || score.S < 0.4) codes.push("noSubject");
  if (!f.hasBoundarySignal || score.B < 0.4) codes.push("noBoundary");
  if (!f.hasBasisSignal || score.K < 0.45) codes.push("noBasis");
  if (!f.hasResponsibilitySignal || score.R < 0.4) codes.push("noResponsibility");

  if (f.hasOfficialClaim && !f.hasOfficialVerificationRoute) codes.push("fakeVerification");

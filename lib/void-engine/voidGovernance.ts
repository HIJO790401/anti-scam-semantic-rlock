...
export function detectVoidGovernance(message: string, f: ExtractedFeatures): GovernanceResult {
  ...
  const fakeProReferral = /(請諮詢專業人士|詢問專家|請洽顧問)/i.test(message) && !f.hasSubjectIdentity;
  const delayEscape = /(我們會改善|我們會修正|將持續優化|後續再處理)/i.test(message) && !/(\d{4}[-/]\d{1,2}[-/]\d{1,2}|負責人|owner|承辦)/i.test(message);
  const fakeNeutrality = /(很多人都這樣認為|全球都在討論|普遍認為)/i.test(message) && (!f.hasSubjectIdentity || !f.hasBoundarySignal || !f.hasResponsibilitySignal);
  const responsibilityEscape = /(我們只是機率式系統|僅供參考不負責|模型可能出錯不承擔)/i.test(message);
